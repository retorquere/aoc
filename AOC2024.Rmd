...
moves <- list(
  ...
  "v" = matrix(c(1, 0), nrow = 1, ncol = 2, byrow = TRUE),
  "<" = matrix(c(0, -1), nrow = 1, ncol = 2, byrow = TRUE),
  ">" = matrix(c(0, 1), nrow = 1, ncol = 2, byrow = TRUE)
)
turn <- list(
  '^' = '>',
...
current <- which(M=='^' | M=='v' | M == '>' | M == '<',arr.ind=TRUE)
direction <- M[current]
visited <- list(current)
while (!is.na(M[current])) {
  nextpos <- current + moves[[direction]]
  while (!is.na(M[nextpos]) && M[nextpos] == '#') {
    direction = turn[[direction]]
    nextpos <- current + moves[[direction]]
  }
  current = nextpos
  if (!is.na(M[current])) visited = unique(append(visited, list(current)))
}
for (current in visited) {
  M[current] = 'X'
}
print(sum(M == "X", na.rm = TRUE))
```