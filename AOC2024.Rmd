...
library(glue)
if (!requireNamespace('zeallot', quietly = TRUE)) { install.packages('zeallot') }
library(zeallot)
...
```{r}
M <- do.call(rbind, strsplit(readLines(here('24/06/sample.txt'), warn=FALSE), ""))
...
walk <- function(m, start) {
  ...
  while (TRUE) {
    ...
    if (is.na(m[guard$pos])) return(list(m, FALSE)) # end of shift
...
c(m, loop) %<-% walk(M, start)

# we have actually visited all these points
print(sum(sapply(m, function(x){ v = suppressWarnings(as.integer(x)); !is.na(v) && bitwAnd(v, 15) != 0 })))
```